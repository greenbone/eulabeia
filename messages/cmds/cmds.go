...
func NewCreate(aggregate string, destination string, groupID string) Create {
	return Create{
		Message: messages.NewMessage(
			buildMessageType("create", aggregate, destination),
			"",
			groupID,
		),
	}
...
//
// The response for Get is usually the aggragte with Message information and can
// be found within a model.
type Get IDCMD
...
func NewGet(
	...
	return Get{
		Identifier: messages.Identifier{
			ID: id,
			Message: messages.NewMessage(
				buildMessageType("get", aggregate, destination),
				"",
				groupID,
			),
		},
...
func NewDelete(
	...
	return Delete{
		Identifier: messages.Identifier{
			ID: id,
			Message: messages.NewMessage(
				buildMessageType("delete", aggregate, destination),
				"",
				groupID,
			),
		},
...
//
// As an example an event with the type start.scan with the id 1 would start
// scan id 1
type Start IDCMD
...
func NewStart(
	...
	return Start{
		Identifier: messages.Identifier{
			ID: id,
			Message: messages.NewMessage(
				buildMessageType("start", aggregate, destination),
				"",
				groupID,
			),
		},
...
//
// The values to modify are within Values, they typically match the names of the
// aggregate
...
func NewModify(
	...
	return Modify{
		Identifier: messages.Identifier{
			ID: id,
			Message: messages.NewMessage(
				buildMessageType("modify", aggregate, destination),
				"",
				groupID,
			),
		},
